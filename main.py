    ...
    for hook in hooks:
        ...
        if keep == ["all"]:
            if arch == "x86":
                ...
                keep = ["rax", "rcx", "rdx", "r8", "r9", "r10", "r11"]
            elif arch == "arm64":
                keep = ["x0", "x1", "x2", "x3", "x4", "x5", "x6", "x7", "x8", "x9", "x10", "x11", "x12", "x13", "x14", "x15", "x29", "x30"]
        ...
        for i in keep:
            add_modrx2_content(ks.asm(f"push {i}", modrx2_address)[0])
        if arch == "x86_64" and len(keep) % 2:
            add_modrx2_content(ks.asm("sub rsp, 0x8", modrx2_address)[0])
        add_modrx2_content(ks.asm(f"call {hex(linked_bin.get_function_address(hook['function']) + input_bin.imagebase)}", modrx2_address)[0])
        keep.reverse()
        if arch == "x86_64" and len(keep) % 2:
            add_modrx2_content(ks.asm("add rsp, 0x8", modrx2_address)[0])
        for i in keep:
            add_modrx2_content(ks.asm(f"pop {i}", modrx2_address)[0])
